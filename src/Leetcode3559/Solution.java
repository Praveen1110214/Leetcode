...
public class Solution {
    ...
    static void dfs(int u, int p, List<Integer>[] graph, int[][] parent, int[] depth){
        parent[0][u] = p;
        for(int v : graph[u]){
            if(v != p){
                depth[v] = depth[u] + 1;
                dfs(u, v, graph, parent, depth);
            }
        }
    }
    static int lca(int u, int v, int[][] parent, int[] depth) {
        if (depth[u] < depth[v]) {
            final int temp = u;
            u = v;
            v = temp;
    ...
    static int modPow(long x, long n){
        if(n == 0) return 1;
        if(n % 2 == 0) return (int) (x * modPow(x % MOD,(n - 1)) % MOD);
